**Global Student Cube: Complete Development Specification**

Version 1.0 \| 14 September 2026 \| Website, Android and iOS

Implementation baseline for UI, UX, frontend, backend, QA and operations. Source: the user-supplied Global-Student-Cube-final-Document.docx. New implementation decisions are explicitly adopted; this is not a claim that software or tests already exist.
...
The selected architecture is a TypeScript monorepo with a separate Next.js responsive website and Expo React Native mobile application, sharing contracts, validation, domain rules, API client and design tokens. Expo documents workspace-based monorepo support; the chosen design deliberately keeps native rendering separate from the web renderer rather than treating Next.js as a native application engine ([<u>Expo monorepo documentation</u>](https://docs.expo.dev/guides/monorepos/)).

The web application and `/api/v1` modular backend deploy to Vercel. Supabase supplies PostgreSQL, authentication, private object storage and authorized realtime; its Next.js guidance covers cookie-based authentication, TypeScript integration and the need to review production row-level policies ([<u>Supabase Next.js guide</u>](https://supabase.com/docs/guides/getting-started/quickstarts/nextjs)).
...
TOEFL iBT reports for tests taken on or after 21 January 2026 use overall and section scores of 1–6 in half-point increments; ETS also supplies a comparable overall 0–120 score during a two-year transition ([<u>ETS score-report guidance</u>](https://www.ets.org/toefl/test-takers/ibt/scores/understand-scores.html)). Therefore require `test_variant`, `scale_code`, `scale_version` and test date alongside the raw reported score. Use `TOEFL_IBT_BAND_2026` for the new scale and `TOEFL_IBT_LEGACY_120` for legacy records; preserve an optional report-provided comparable total as a separate field, never overwrite the band score. Populate the date-appropriate default while allowing a verified historical edition. Each program requirement stores its own accepted scale/version. Cross-scale evaluation requires an explicitly published institution rule or a versioned, reviewed concordance; without one, return “Manual review needed,” not an invented arithmetic conversion. Acceptance fixtures must cover pre-change reports, post-change band scores, transition reports containing both totals, and a program whose requirement still uses the legacy scale.
...
2.  `.relevance` and `.prepared_knowledgeable`: Strongly Agree, Agree, Neutral, Disagree.

3.  `.action_clarity`: Yes, very clear; Somewhat clear; Minimal; None.

4.  `.satisfaction`: Very Satisfied, Satisfied, Neutral, Unsatisfied.

5.  `.challenges[]`: Lack of clarity; Information overload; Too fast/too slow pace; Not enough time; Limited topic knowledge; Technical issues (audio/call quality); Difficulty understanding process/steps; None. None excludes every other selection.
...
| `counselor.experience_band`, `.experience_years`                                                                               | R band/numeric 0–80                           | Less than 2; 2 to 5; 5 to 10; 10 to 20; More than 20. Exact boundaries: .                                                                                                           |
...
2.  Add one unseen university in the first manual university's city; absent manual anchor, use the highest-priority preferred city. Skip when no anchor or candidate exists.

3.  Add one unseen university in the anchor country, or priority-one country without a manual anchor.

4.  Add up to four unseen candidates with the lowest known comparable annual costs.

5.  Add up to three unseen candidates with strongest comparable course rankings.

6.  Fill remaining slots with unseen candidates ordered by country priority, known annual cost, course rank and UUID. Unknown costs/ranks sort after known values within their respective comparisons.
...
Expo documents workspace-based monorepo support; this supports the selected repository structure, not a claim that every dependency works unchanged on native platforms ([<u>Expo monorepo guide</u>](https://docs.expo.dev/guides/monorepos/)). Supabase’s Next.js quickstart documents cookie-based authentication, TypeScript/Tailwind setup, and row-level security; the stricter BFF and authorization design below remains an authored implementation requirement ([<u>Supabase Next.js quickstart</u>](https://supabase.com/docs/guides/getting-started/quickstarts/nextjs)).
